...
void hessian_fd2(double (*funcpt)(double *,int),double *x,int N,double *dx,double *f) {
	int i,j,step;
	double fd,temp,maxstep,ft,fc,temp2,stepmax;
	double *stepsize,*f2;
...
int trsrch(double (*funcpt)(double *,int),double *xi,double *jac,double *sN,int N,double * dx,double maxstep,
	...
	while (retval >= 2) {
		trstep(jac,sN,N,dx,L,hess,nlen,ioval,step);
		ioval[2] = ioval[0];
		retval = trupdate(funcpt,xi,jac,step,N,dx,maxstep,retval,L,hess,stol,method,ioval,xprev,x);
	}
...
int trupdate(double (*funcpt)(double *,int),double *xi,double *jac,double *step,int N,double * dx,double maxstep,
	...
	double funci,funcf,df,funcprev,dfp,temp;
...
int newton_min_trust(double (*funcpt)(double *,int),double *xi,int N,double *dx,double fsval,double delta,int method,double *xf) {
	...
	while (rcode == 0 && iter < siter) {
		...
		scale(jac,1,N,-1.0);
		if (method == 0) {
			retval = trsrch(funcpt,xc,jac,step,N,dx,maxstep,iter,L,hess,stol,ioval,xf);
		} else if (method ==1) {
